...
void load_memory(VM* vm) {
    vm->ram_memory = (uint8_t*)malloc(0x4000);
    memset(vm->ram_memory, 0, 0x4000);
}
...
void run_vm(VM* vm) {
    ...
    while(1) {
                    ...
                    switch(format_specifier) {
                        ...
                        case xb: {
                            for(int i=0; i<length; i++) {
                                printf("%02x", *(uint8_t*)(vm->ram_memory + DATA_SEGMENT + offset + i));
                            }
                            fflush(stdout);
                            break;
                        }
        ...
        if (opcode != OP_CALL && opcode != OP_RETURN && opcode != OP_IFGOTO) {
            if(micro_program_addr != (uint16_t)-1) {
                execute_micro_program(vm, micro_program_addr);
            } else {
                puts("UNKNOWN OPCODE");
                return;
